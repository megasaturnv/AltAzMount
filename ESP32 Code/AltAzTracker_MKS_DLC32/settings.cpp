...
//const double ALT_STEPPER_STEPS_PER_REV                        = 144000.0; // For design 1. 200 (steps) * 16 (1/16 microstepping) * 90 (gear teeth) / 2 (2-start worm gear) = 360 degrees (0.0025° per step)
//const double AZ_STEPPER_STEPS_PER_REV                         = 144000.0; // For design 1. 200 (steps) * 16 (1/16 microstepping) * 90 (gear teeth) / 2 (2-start worm gear) = 360 degrees (0.0025° per step)
const double ALT_STEPPER_STEPS_PER_REV                        = 9752.38095238095238; // For design 2.  200 (steps) * 16 (1/16 microstepping) * 64 (big gear teeth) / 21 (small gear teeth) = 360 degrees (0.0025° per step)
const double AZ_STEPPER_STEPS_PER_REV                         = 27105.8823529411764; // For design 2.  200 (steps) * 16 (1/16 microstepping) * 144 (big gear teeth) / 17 (small gear teeth) = 360 degrees (0.0025° per step)
const int WITHIN_STEPS_TO_ARRIVE_AT_TARGET                    = 16;
const float ANGLE_TO_MOVE_OFF_DEPRESSED_LIMIT_SWITCH          = 3.0; // Move 2 degrees away from limit switch if it is depressed while trying to home
const float ANGLE_ALLOWED_WITHIN_LIMIT_SWITCH_BEING_DEPRESSED = 3.0; // Within this angle of the limit switch, ignore if the limit switch is pressed as this is expected
//const bool STEPPER_ALT_MOVE_IN_POSITIVE_DIRECTION             = false; // For design 1
//const bool STEPPER_AZ_MOVE_IN_POSITIVE_DIRECTION              = true; // For design 1
const bool STEPPER_ALT_MOVE_IN_POSITIVE_DIRECTION             = true; // For design 2
const bool STEPPER_AZ_MOVE_IN_POSITIVE_DIRECTION              = false; // For design 2

const float X_STEPPER_MINIMUM_ANGLE              = 0.0; // Home posiiton should always be set at 0. No need to change this
const float X_STEPPER_MAXIMUM_ANGLE              = 360.0;
const float X_STEPPER_ANGLE_OF_NORTH_FROM_HOME   = 0.0;
const float X_STEPPER_ANGLE_OF_SOUTH_FROM_HOME   = 180.0;

const float Z_STEPPER_MINIMUM_ANGLE              = 0.0; // Home posiiton should always be set at 0. No need to change this
//const float Z_STEPPER_MAXIMUM_ANGLE              = 130.0; // For design 1
//const float Z_STEPPER_ANGLE_OF_HORIZON_FROM_HOME = 35.0; // For design 1
//const float Z_STEPPER_ANGLE_OF_ZENITH_FROM_HOME  = 125.0; // For design 1
const float Z_STEPPER_MAXIMUM_ANGLE              = 180.0; // For design 2
const float Z_STEPPER_ANGLE_OF_HORIZON_FROM_HOME = 18.0; // For design 2
const float Z_STEPPER_ANGLE_OF_ZENITH_FROM_HOME  = 108.0; // For design 2
...
//Pins assigned to Motors and limit switches (For design 1)
/*
...
//Pins assigned to Motors and limit switches (For design 2)
const int SR_ALT_STEP_BIT    = SR_Y_STEP_BIT;
const int SR_ALT_DIR_BIT     = SR_Y_DIR_BIT;
const int SR_AZ_STEP_BIT     = SR_Z_STEP_BIT;
const int SR_AZ_DIR_BIT      = SR_Z_DIR_BIT;
const int PIN_ALT_LIMIT_HOME = PIN_Z_LIMIT;
const int PIN_ALT_LIMIT_END  = PIN_Z_PROBE;
const int PIN_AZ_LIMIT_HOME  = PIN_X_LIMIT;
const int PIN_AZ_LIMIT_END   = PIN_Y_LIMIT;
